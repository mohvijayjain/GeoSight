...
MODEL_PATH = r"C:\Users\Mohvijay-sch\Desktop\GeoSight2\checkpoints\geosight_final_epoch_11.pt"
OUTPUT_DIR = r"C:\Users\Mohvijay-sch\Desktop\GeoSight2\Indore_predictions_epoch11_filtered"
VISUAL_DIR = r"C:\Users\Mohvijay-sch\Desktop\GeoSight2\Indore_visual_inspection_epoch11_filtered"
BATCH_SIZE = 32
NUM_SAMPLES = 20
MIN_WATER_SIZE = 500
...
def test_indore_epoch11_filtered():
    print(f"{'='*70}")
    print(f"🏙️  TESTING EPOCH 11 ON INDORE WITH WATER FILTERING")
    print(f"{'='*70}")
    ...
    if not os.path.exists(MODEL_PATH):
        print(f"\n❌ ERROR: Epoch 11 model not found!")
        return
    ...
    print(f"\n{'='*70}")
    print(f"✅ Testing Complete - Epoch 11 on Indore")
    print(f"{'='*70}")
    ...
    if water_after < 12:
        ...
        print(f"      Appropriate for inland city (Indore)")
    elif water_after > 15:
        print(f"\n  ⚠️  Water still at {water_after:.2f}%")
        print(f"      Consider increasing MIN_WATER_SIZE or using 8-band model")
    ...
    print(f"\n🏙️  Indore Characteristics:")
    print(f"   - Central Indian plains city")
    print(f"   - Inland location (limited water bodies)")
    print(f"   - Expected: High urban, moderate rural, low water")
