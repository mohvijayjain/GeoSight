"""
Test Road Detection Model on Dehradun Tiles
...
DEHRADUN_DIR = r"C:\Users\Mohvijay-sch\Desktop\GeoSight2\Dehradun_tiles"
OUTPUT_DIR = r"C:\Users\Mohvijay-sch\Desktop\GeoSight2\Dehradun_Road_Results"
NUM_SAMPLES = 10
...
def create_visualization(original, road_mask, filename):
    ...
    ax1.imshow(original)
    ax1.set_title('Dehradun Satellite Image', fontsize=16, fontweight='bold', pad=15)
    ax1.axis('off')
    ...
    # Add title
    fig.suptitle(f'Dehradun Road Detection: {filename}', 
                 fontsize=18, fontweight='bold', y=0.98)
    
    return fig

# Get Dehradun images
print(f"\nScanning Dehradun_tiles folder...")
all_files = [f for f in os.listdir(DEHRADUN_DIR) if f.lower().endswith(('.tif', '.png', '.jpg'))]
print(f"Found {len(all_files)} images in Dehradun_tiles")

if len(all_files) == 0:
    print("ERROR: No images found in Dehradun_tiles folder!")
    exit(1)
...
print(f"\nGenerating {num_to_process} professional visualizations for Dehradun...")
print("="*60)
...
for i, filename in enumerate(test_files):
    ...
    img_path = os.path.join(DEHRADUN_DIR, filename)
    ...
    # Save
    output_filename = filename.replace('.tif', '').replace('.png', '').replace('.jpg', '') + '_DEHRADUN_RESULT.png'
...
print("\n" + "="*60)
print(f"Successfully generated {success_count} Dehradun road visualizations")
print(f"Output directory: {OUTPUT_DIR}")
print("="*60)
print("\nThese visualizations show road detection results for Dehradun!")
