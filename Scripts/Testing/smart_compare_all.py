...
PREVIOUS_RESULTS_DIR = r"C:\Users\Mohvijay-sch\Desktop\GeoSight2\all_epochs_analysis"
OUTPUT_DIR = r"C:\Users\Mohvijay-sch\Desktop\GeoSight2\complete_model_comparison"
BATCH_SIZE = 32
...
def load_previous_results():
    """Load previously tested results if available"""
    previous_results_path = os.path.join(PREVIOUS_RESULTS_DIR, "all_epochs_results.csv")
    
    if os.path.exists(previous_results_path):
        df = pd.read_csv(previous_results_path)
        # Add model_name and model_type if not present
        if 'model_name' not in df.columns:
            df['model_name'] = df['epoch'].apply(lambda x: f"Epoch {int(x)}")
...
def test_single_model(model_info, dataloader):
    ...
    with torch.no_grad():
        for images, names in tqdm(dataloader, desc=f"Testing {model_info['model_name']}", leave=True):
...
def create_comprehensive_plot(df_all):
    ...
    ax1.scatter([best['sort_key'] if best['model_type'] != 'epoch' else best['epoch']], 
               [best['quality_score']], color='red', s=400, marker='*', 
               zorder=10, edgecolors='gold', linewidths=2)
    ax1.set_xlabel('Epoch / Model', fontsize=11)
    ax1.set_ylabel('Quality Score', fontsize=11)
    ax1.set_title('Quality Score Comparison', fontsize=13, fontweight='bold')
    ...
    ax2.set_title('Urban Detection', fontsize=13, fontweight='bold')
    ...
    ax3.set_title('Rural Detection', fontsize=13, fontweight='bold')
    ...
    ax4.set_title('Water Detection', fontsize=13, fontweight='bold')
    ...
    ax7.set_title('Top 10 Models', fontsize=13, fontweight='bold')
    ax7.invert_yaxis()
    ...
    table = ax9.table(cellText=table_data, 
                     colLabels=['Rank', 'Model', 'Score', 'Urban%', 'Water%'],
                     cellLoc='center', loc='center',
                     colWidths=[0.1, 0.3, 0.2, 0.2, 0.2])
    table.auto_set_font_size(False)
    table.set_fontsize(9)
    table.scale(1, 2)
    ax9.set_title('Top 5 Summary', fontsize=13, fontweight='bold', pad=20)
    
    plt.tight_layout()
    plot_path = os.path.join(OUTPUT_DIR, "complete_model_comparison.png")
    plt.savefig(plot_path, dpi=150, bbox_inches='tight')
    plt.close()
    
    return plot_path

def smart_test_and_compare():
    print(f"{'='*70}")
    print(f"🚀 SMART MODEL COMPARISON: All Checkpoints")
    print(f"{'='*70}")
    ...
    print(f"\n{'='*70}")
    print("📊 Checking for previous test results...")
    df_previous = load_previous_results()
    
    tested_epochs = set()
    if df_previous is not None:
        tested_epochs = set(df_previous['epoch'].values)
        print(f"✅ Found {len(tested_epochs)} previously tested epoch models")
    else:
        print(f"⚠️  No previous results found - will test all models")
    ...
    for model_info in all_models_info:
        if model_info['model_type'] == 'epoch' and model_info['epoch'] in tested_epochs:
            models_already_tested.append(model_info)
        else:
            models_to_test.append(model_info)
    
    print(f"\n{'='*70}")
    print(f"📋 Testing Summary:")
    print(f"   ✅ Already tested: {len(models_already_tested)} models")
    print(f"   🔄 Need to test: {len(models_to_test)} models")
    
    if models_to_test:
        print(f"\n🔄 Models to test:")
        for m in models_to_test:
            print(f"   - {m['model_name']} ({m['filename']})")
    
    # Load dataset
    dataset = TestDataset(TEST_IMG_DIR)
    dataloader = DataLoader(dataset, batch_size=BATCH_SIZE, num_workers=4, pin_memory=True)
    print(f"\n📂 Test dataset: {len(dataset)} images")
    ...
    if models_to_test:
        ...
        for idx, model_info in enumerate(models_to_test, 1):
            print(f"{idx}/{len(models_to_test)}: {model_info['model_name']}")
    ...
    # Combine all results
    if df_previous is not None:
        # Add sort_key to previous results
        df_previous['sort_key'] = df_previous['epoch']
        df_previous['filename'] = df_previous.apply(
            lambda x: f"geosight_final_epoch_{int(x['epoch'])}.pt", axis=1
        )
        df_all = pd.concat([df_previous, pd.DataFrame(new_results)], ignore_index=True)
    else:
        df_all = pd.DataFrame(new_results)
    
    # Save results
    csv_path = os.path.join(OUTPUT_DIR, "complete_model_comparison.csv")
    ...
    print(f"\n🏆 ABSOLUTE BEST MODEL: {best['model_name']}")
    print(f"{'='*70}")
    ...
    if best['water'] > 25:
        ...
        print(f"      Verify visually for Indore accuracy")
    
    if best['urban_rural_total'] > 65:
        print(f"\n  ✅ Excellent land detection: {best['urban_rural_total']:.1f}%")
    
    if best['background'] < 10:
        print(f"\n  ✅ Low background: {best['background']:.1f}% (confident predictions)")
