...
# --- CONFIG ---
TEST_IMG_DIR = r"C:\Users\Mohvijay-sch\Desktop\GeoSight2\Dehradun_Indore_tiles"
CHECKPOINTS_DIR = r"C:\Users\Mohvijay-sch\Desktop\GeoSight2\checkpoints"
OUTPUT_DIR = r"C:\Users\Mohvijay-sch\Desktop\GeoSight2\Dehradun_Indore_all_models_analysis"
BATCH_SIZE = 32
...
def calculate_quality_score_combined(class_pcts):
    """
    Calculate quality score for combined Dehradun+Indore dataset
    Balanced criteria for diverse geography
    """
    ...
    # Urban + Rural: prefer 60-80% (balanced for both cities)
    ...
    # Water: prefer 10-20% (average of both cities)
    if 10 <= water <= 20:
        score += 30
...
def create_comprehensive_plot(df_all):
    ...
    ax1.set_title('Quality Score - Combined Dataset', fontsize=13, fontweight='bold')
    ...
    ax4.axhline(y=15, color='cyan', linestyle=':', label='Target: ~15%', linewidth=2)
    ...
    plt.tight_layout()
    plot_path = os.path.join(OUTPUT_DIR, "dehradun_indore_all_models_comparison.png")
...
def test_all_models_combined():
    print(f"{'='*70}")
    print(f"🌍 COMPREHENSIVE TESTING: ALL MODELS ON COMBINED DATASET")
    print(f"{'='*70}")
    print(f"📍 Dataset: Dehradun + Indore (Combined Geography)")
    print(f"Device: {torch.cuda.get_device_name(0) if torch.cuda.is_available() else 'CPU'}\n")
    
    # Check if Dehradun_Indore_tiles exists
    if not os.path.exists(TEST_IMG_DIR):
        print(f"❌ ERROR: Dehradun_Indore_tiles directory not found!")
    ...
    print(f"📂 Combined dataset: {len(dataset)} images\n")
    ...
    print("🔄 Testing all models on combined dataset...")
    print(f"{'='*70}\n")
    ...
    for idx, model_info in enumerate(tqdm(all_models_info, desc="Overall Progress"), 1):
        ...
        quality_score = calculate_quality_score_combined(class_pcts)
    ...
    # Save results
    csv_path = os.path.join(OUTPUT_DIR, "dehradun_indore_all_models_results.csv")
    ...
    print(f"\n{'='*70}")
    print(f"✅ COMBINED DATASET TESTING COMPLETE!")
    print(f"{'='*70}")
    
    print(f"\n🏆 BEST MODEL FOR COMBINED DATASET: {best['model_name']}")
    print(f"{'='*70}")
    ...
    print(f"\n🥇 TOP 10 MODELS FOR COMBINED DATASET:")
    print(f"{'='*70}")
    ...
    print(f"   📍 Dehradun: Himalayan foothill city (forests, rivers)")
    print(f"   📍 Indore: Central Indian plains city (urban, moderate water)")
    print(f"   🎯 Combined: Tests model generalization across diverse geography")
    
    if best['urban_rural_total'] > 65:
        print(f"\n  ✅ Excellent land detection: {best['urban_rural_total']:.1f}%")
        print(f"      (Good generalization across both cities)")
    
    if best['background'] < 10:
        print(f"\n  ✅ Low background: {best['background']:.2f}%")
        print(f"      (Confident predictions on diverse terrain)")
    
    if 10 <= best['water'] <= 20:
        print(f"\n  ✅ Balanced water detection: {best['water']:.2f}%")
        print(f"      (Appropriate for combined dataset)")
    elif best['water'] > 25:
        print(f"\n  ⚠️  High water detection: {best['water']:.2f}%")
        print(f"      (May need visual verification)")
    
    print(f"\n🎯 Model Generalization:")
    print(f"   This model performs best across BOTH:")
    print(f"   - Plains cities (Indore)")
    print(f"   - Himalayan cities (Dehradun)")
    print(f"   → Best choice for diverse Indian geography!")
