"""
Test Road Detection Model on Kanpur Tiles
...
KANPUR_DIR = r"C:\Users\Mohvijay-sch\Desktop\GeoSight2\Kanpur_tiles_8band_overlap_50"
OUTPUT_DIR = r"C:\Users\Mohvijay-sch\Desktop\GeoSight2\Kanpur_Road_Results"
NUM_SAMPLES = 20
...
def load_image(img_path):
    ...
    if img_path.lower().endswith('.tif'):
        with rasterio.open(img_path) as src:
            # Read RGB bands (3, 2, 1 for true color from 8-band)
            if src.count >= 3:
                img = src.read([3, 2, 1]).transpose(1, 2, 0)
            else:
                img = src.read([1, 1, 1]).transpose(1, 2, 0)
...
def create_visualization(original, road_mask, filename):
    ...
    ax1.imshow(original)
    ax1.set_title('Kanpur Satellite Image', fontsize=16, fontweight='bold', pad=15)
    ax1.axis('off')
    ...
    # Add title
    fig.suptitle(f'Kanpur Road Detection: {filename}', 
                 fontsize=18, fontweight='bold', y=0.98)
    
    return fig

# Get Kanpur images
print(f"\nScanning Kanpur_tiles_8band_overlap_50 folder...")
all_files = [f for f in os.listdir(KANPUR_DIR) if f.lower().endswith(('.tif', '.png', '.jpg'))]
print(f"Found {len(all_files)} images in Kanpur_tiles_8band_overlap_50")

if len(all_files) == 0:
    print("ERROR: No images found in Kanpur_tiles_8band_overlap_50 folder!")
    exit(1)
...
print(f"\nGenerating {num_to_process} professional visualizations for Kanpur...")
print("="*60)
...
for i, filename in enumerate(test_files):
    ...
    img_path = os.path.join(KANPUR_DIR, filename)
    ...
    # Save
    output_filename = filename.replace('.tif', '').replace('.png', '').replace('.jpg', '') + '_KANPUR_RESULT.png'
...
print("\n" + "="*60)
print(f"Successfully generated {success_count} Kanpur road visualizations")
print(f"Output directory: {OUTPUT_DIR}")
print("="*60)
print("\nThese visualizations show road detection results for Kanpur!")
