...
OUTPUT_DIR = r"C:\Users\Mohvijay-sch\Desktop\GeoSight2\Kanpur_predictions_epoch16_filtered"
VISUAL_DIR = r"C:\Users\Mohvijay-sch\Desktop\GeoSight2\Kanpur_visual_inspection_epoch16_filtered"
BATCH_SIZE = 32
NUM_SAMPLES = 20
...
def filter_water_predictions(predictions):
    ...
    Real water bodies (rivers, lakes) are large connected regions.
    Small isolated water pixels are likely misclassified buildings/shadows.
    """
    # Create water mask (class 3)
    water_mask = (predictions == 3)
    
    # Remove small water objects (< MIN_WATER_SIZE pixels)
    # Real rivers/lakes are large, small blue dots are errors
    ...
    # Where water was removed, replace with most likely alternative class
    # Check what was originally predicted before water
    removed_water = water_mask & ~clean_water
    
    # For removed water pixels, set to Urban (class 2) 
    # since buildings are most commonly confused with water
    filtered_pred[removed_water] = 2  # Urban
    ...
    with torch.no_grad():
        for images, names, rgbs in tqdm(dataloader, desc="Processing"):
            ...
            for i, pred_raw in enumerate(preds_raw):
                ...
                # Count classes for both raw and filtered
...
def visualize_samples(predictions_raw, predictions_filtered, filenames, rgb_images):
    ...
    for idx, sample_idx in enumerate(sample_indices):
        ...
        # Calculate class percentages for both
        ...
        # Panel 2: Raw prediction (before filtering)
        ...
        # Panel 3: Filtered prediction (after water filtering)
        ...
        # Panel 4: Overlay of filtered prediction
...
def test_kanpur_epoch16_filtered():
    print(f"{'='*70}")
    print(f"🏭 TESTING EPOCH 16 WITH WATER FILTERING")
    print(f"{'='*70}")
    print(f"Device: {torch.cuda.get_device_name(0)}")
    print(f"\n💡 Water Filtering Strategy:")
    print(f"   - Remove water bodies smaller than {MIN_WATER_SIZE} pixels")
    print(f"   - Real rivers/lakes are large connected regions")
    print(f"   - Small isolated 'water' pixels are likely buildings/shadows")
    print(f"   - Reclassify removed water as Urban (buildings)\n")
    ...
    print(f"\n{'='*70}")
    print(f"✅ Testing Complete - Epoch 16 with Water Filtering")
    print(f"{'='*70}")
    ...
    if water_after < water_before * 0.7:
        ...
        print(f"  ℹ️  No change - water bodies may already be correctly classified")
    
    if water_after > 15:
        print(f"\n  ⚠️  Water still at {water_after:.2f}%")
        print(f"      Consider:")
        print(f"      1. Increasing MIN_WATER_SIZE (currently {MIN_WATER_SIZE})")
        print(f"      2. Training an 8-band model for better spectral separation")
    else:
        print(f"\n  ✅ Water detection now realistic: {water_after:.2f}%")
        print(f"      Appropriate for Ganges river in Kanpur")
